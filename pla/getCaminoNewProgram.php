...
function generaCamino(data,piso)
{
	...
	if(piso=="1")
		{
			var matrix = gMatrix1();
			var grid = new PF.Grid(137, 81, matrix);
		}
	if(piso=="2")
		{
			var matrix = gMatrix2();
...
function generaMapa(camino)
{
	...
	var otrosVolumenes = [1000,1001,1003,2000,2001,2002,3000,3001,4000,4001];
	var totalPisos =4;
	/*
	...
	*/
	...
	if(Inicio>0)
	{
		...
		{ // carga losa de el arreglo que dice otrosVolumenes
			if(parseInt(otrosVolumenes[i]) < maxValorVolumen)
			{
	...
	else
	{
		...
		for (var i = 0; i < otrosVolumenes.length; i++) 
		{ // carga losa de el arreglo que dice otrosVolumenes
			if(maxValorVolumen >= parseInt(otrosVolumenes[i]) )
			{
				localesPisoInical.push(parseInt(otrosVolumenes[i]));		
			}	
		};
	...
	*/
	...
	function init() {
		...
		*/
		...
		matPisoA = [activo, pasivo, pisoColor, pisoColor1,sLaPolar,sCinemundo, sFalabella, sParis, sRipley, sHomecenter, sLider, sCinemark, sEstac, sBoulevard, sTerrazas, sAutoplaza, sAires,sJohnson];
		...
		matPisoB = [activo2, pasivo2, pisoColor2,sEstac2,sCinemundo2,sFalabella2,sLaPolar2,sJohnson2,sAutoplaza2, sParis2, sRipley2, sCinemark2, sCinemark , sEstac2, sE_Azul, sE_Cyan, sE_Gris, sE_Rojo, sE_Verde, sComidas, sAires2, sBoulevard0];
		...
		*/
		...
		if(totem.piso == 1)
		{
			...
			*/
			...
			if(Meta!=0)
			{
				//console.log("Tiendas Anclas Meta "+tiendasAnclaMeta);
				if(Meta>0)
				{
					losaPiso = Meta*1000;
				}
					...
					for (i=0; i<o; i++)
					{
						...
						if( pisoMeta.children[i].name === idTienda ) 
						{
					 	...
					 	if(parseInt(pisoMeta.children[i].name)>=losaPiso)
					 	{
					 		...
					 		for (var j = 0; j < otrosVolumenes.length; j++) 
					 		{
					 			...
					 			if(pisoMeta.children[i].name == otrosVolumenes[j] && (losaPiso+2) == pisoMeta.children[i].name )
					 			{
					 				//console.log(pisoInicio.children[i].name);
					 				pisoMeta.children[i].material = sEstac2;
					 				bandera =3;	
					 			}
					 		}
					 	}			
					 	 ...
					 	 if(mTerrazas && mTerrazas.length>0)
						{
							for (var t = 0; t < mTerrazas.length; t++) 
							{
								...
								{
									pisoMeta.children[i].material = sTerrazas;
									bandera =3
								}
							}
						}	
						...
						if(mComidas && mComidas.length>0)
						{
							...
							for (var t = 0; t < mComidas.length; t++) 
							{
								...
								{
									//console.log("aqui pitando volumen");
									pisoMeta.children[i].material = sComidas;
									bandera =3
								}
							}
						}
						...
						if(mBoulevard && mBoulevard.length>0 )
						{
							...
							for (var t = 0; t < mBoulevard.length; t++) 
							{
								...
								{
									pisoMeta.children[i].material = sBoulevard;
									bandera =3
								}
							}
						}
						...
						if(mAires && mAires.length>0)
						{
							for (var t = 0; t < mAires.length; t++) 
							{
								...
								{
									pisoMeta.children[i].material = sAires;
									bandera =3
								}
							}
						}
       ...
       // console.log(piso_T);
		...
		if (camino[3] && Meta>=2 )//(parseInt(camino[3]) == 2 || parseInt(camino[3]) == 3) )  
		{
			dB1[5]	= [224 , 49, -44];
		} 
		else if (camino[3] && Meta<0) //parseInt(camino[3]) == -1) 
		{
			dB1[5]	= [224 , 15, -44];
		};
	...
	function prepare() {
		...
		loader.load( "src/js/mapa/PLA_gral_v01.js", callbackFinished );
